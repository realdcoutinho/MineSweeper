...
void Start()
{
	...
	//TwoDArrays(g_Rows, g_Colums, g_GridArray);
...
void TwoDArrays(const int rows, const int columns, int* pArray)
{
	...
	for (int row{ 0 }; row < rows; row++)
	{
		for (int column{ 0 }; column < columns; column++)
		{
			...
			pArray[index] = (row + 1) * columns + column + 1;
		}
	}
...
void Print2DElements(int* pNumbers, int rows, int columns)
{
	for (int row = 0; row < rows; row++)
	{
		...
		{
			int index{ GetIndex(row, column, columns) };
			std::cout << std::setw(5) << pNumbers[index];
		}
		std::cout << std::endl;
	}
}
